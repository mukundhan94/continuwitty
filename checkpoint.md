...
- **Go migration current checkpoint (2026-02-26):** CP121 completed in `checkpoint-go-migration.md` with `chat.list_project_documents` MCP dispatch in Go compatibility mode across direct and `tools/call` paths (with optional `project_id` filter, paging defaults, and runtime document-list adapter wiring), full `go test ./...` pass, and CodeScene safeguard pass.
