...
- **Go migration current checkpoint (2026-02-26):** CP102 completed in `checkpoint-go-migration.md` with MCP stream actor-auth baseline in Go (bearer-token resolution + session fallback, OAuth-aware unauthorized headers, actor propagation into stream requests), runtime/router dependency wiring, full `go test ./...` pass, and CodeScene safeguard pass.
