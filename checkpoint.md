...
- **Go migration current checkpoint (2026-02-26):** CP126 completed in `checkpoint-go-migration.md` with `chat.restore_session` MCP dispatch in Go compatibility mode across direct and `tools/call` paths, plus owner/admin access hardening for both restore and delete lifecycle flows, full `go test ./...` pass, and CodeScene safeguard pass.
