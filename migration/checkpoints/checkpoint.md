...
- **Phase 41 in progress:** explicit feedback enrichment now supports optional `relevance_score` and optional `session_id` attribution across REST/MCP feedback submission with persisted `feedback_count` and `avg_relevance_feedback` aggregate tracking.
...
- [x] Extended feedback repository write path:
  - feedback events now persist optional `relevance_score` and optional `session_id`.
...
- [x] Added REST + MCP feedback payload parity:
  - REST and MCP feedback requests now accept optional `relevance_score` with `1-5` validation and optional `session_id` UUID.
  - MCP `engram.feedback` catalog schema now documents `relevance_score` and `session_id`.
- [x] Added regression coverage:
  - repository tests for relevance score persistence + validation and aggregate returns.
  - API route tests for relevance-score forwarding and validation.
  - MCP compatibility tests for relevance-score forwarding and validation.
