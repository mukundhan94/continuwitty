...
- **Go migration current checkpoint (2026-02-26):** CP115 completed in `checkpoint-go-migration.md` with `chat.list_pinned_engrams` MCP dispatch in Go compatibility mode across direct and `tools/call` paths (with required session lookup and runtime pinned-engram wiring), full `go test ./...` pass, and CodeScene safeguard pass.
