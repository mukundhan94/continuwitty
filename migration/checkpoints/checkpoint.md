...
- **Phase 39 in progress:** temporal query + cost-aware context baseline shipped with new `engram.query` filters (`access_count_min`, `freshness_score_min`) and bounded `context_token_budget` controls across REST/MCP chat send paths; retrieval audit now captures budget diagnostics (`context_token_budget`, `context_token_estimate`, `context_token_truncated`) and advanced timeline/trace windows remain.
...
- [ ] Add advanced timeline/trace temporal filters and contract coverage.
