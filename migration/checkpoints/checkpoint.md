...
- **Phase 41 in progress:** explicit feedback enrichment now supports optional `relevance_score` across REST/MCP feedback submission with persisted `feedback_count` and `avg_relevance_feedback` aggregate tracking.
...
- [x] Extended feedback repository write path:
  - feedback events now persist optional `relevance_score`.
...
- [x] Added REST + MCP feedback payload parity:
  - REST and MCP feedback requests now accept optional `relevance_score` with `1-5` validation.
  - MCP `engram.feedback` catalog schema now documents `relevance_score`.
