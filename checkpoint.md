...
- **Go migration current checkpoint (2026-02-27):** CP167 completed in `checkpoint-go-migration.md` with MCP stream-route token project-scope denial parity extension for transfer tools (`project.export_bundle`, `project.import_bundle`) across `tools/call` and direct methods, full `go test ./...` pass, and CodeScene safeguard pass.
