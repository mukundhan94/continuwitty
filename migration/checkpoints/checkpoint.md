...
- **Phase 40 in progress:** autonomous memory suggestions now include schema/model/repository foundations, admin REST + MCP list/action parity (`engram.curation_list`, `engram.curation_action`), deterministic generation hooks from consolidation/contradiction refresh workflows, and mock acceptance coverage for curation generation/action flows; benchmark quality/latency coverage remains.
...
- [ ] Add benchmark coverage for curation suggestion precision/recall and action latency.
