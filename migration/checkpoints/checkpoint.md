...
- **Phase 39 in progress:** cost-aware context assembly baseline shipped with bounded `context_token_budget` controls across REST/MCP chat send paths and retrieval-audit token-budget diagnostics (`context_token_budget`, `context_token_estimate`, `context_token_truncated`); temporal query-extension filters remain.
...
- [ ] Add temporal query-extension filters and contract coverage.
