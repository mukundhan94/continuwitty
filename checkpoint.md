...
- **Go migration current checkpoint (2026-02-26):** CP117 completed in `checkpoint-go-migration.md` with `chat.pin_engram` / `engram.pin_to_session` MCP dispatch in Go compatibility mode across direct and `tools/call` alias paths (with required UUID validation, runtime pin adapter wiring, and code-health-safe dispatch modularization), full `go test ./...` pass, and CodeScene safeguard pass.
