...
- **Go migration current checkpoint (2026-02-26):** CP123 completed in `checkpoint-go-migration.md` with `chat.update_lifecycle_policy` MCP dispatch in Go compatibility mode across direct and `tools/call` paths (with required `session_id`, optional partial lifecycle fields, runtime lifecycle-update adapter wiring, and not-found error mapping), full `go test ./...` pass, and CodeScene safeguard pass.
