...
- **Go migration current checkpoint (2026-02-27):** CP137 completed in `checkpoint-go-migration.md` with MCP project transfer dispatch baseline (`project.export_bundle`, `project.import_bundle`) in Go compatibility mode across direct and `tools/call` paths, runtime export/import service wiring, bundle/conflict-policy validation parity, export/import error mapping parity (`404`/`422`), full `go test ./...` pass, and CodeScene safeguard pass.
