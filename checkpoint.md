...
- **Go migration current checkpoint (2026-02-27):** CP149 completed in `checkpoint-go-migration.md` with MCP token-policy parity test hardening for no-session `chat.save_as_engram` (single-project autofill + multi-project explicit-project validation), full `go test ./...` pass, and CodeScene safeguard pass.
