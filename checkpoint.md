...
- **Go migration current checkpoint (2026-02-27):** CP156 completed in `checkpoint-go-migration.md` with MCP stream-route token project-scope denial parity for create tools (`engram.create`, `engram.create_from_conversation`, no-session `chat.save_as_engram`) across `tools/call` and direct methods, full `go test ./...` pass, and CodeScene safeguard pass.
