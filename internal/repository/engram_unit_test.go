...
func queryWhereExpectedFragments() []string {
	return []string{
		...
		"COALESCE(feedback_count, 0) >= $11",
		"COALESCE(feedback_count, 0) <= $12",
		"COALESCE(contradiction_count, 0) <= $13",
		"THEN 0.0",
		"ELSE COALESCE(contradiction_count, 0)::DOUBLE PRECISION /",
		") <= $14",
		"COALESCE(freshness_score, 1.0) >= $15",
		"CASE",
		"THEN 0.5",
		"ELSE COALESCE(useful_count, 0)::DOUBLE PRECISION /",
		"GREATEST(COALESCE(feedback_count, 0), 1)::DOUBLE PRECISION",
		") >= $16",
		"COALESCE(avg_relevance_feedback, 0.5) >= $17",
		"COALESCE(source_session_quality_score, 0.5) >= $18",
		"COALESCE(last_accessed_at, created_at) >= $19",
		"COALESCE(last_accessed_at, created_at) <= $20",
		"COALESCE(freshness_last_computed_at, created_at) >= $21",
		"COALESCE(freshness_last_computed_at, created_at) <= $22",
		"EXISTS (",
		"link.status = 'active'",
		"link.relation_type = $23",
		"owner_user_id = $3",
