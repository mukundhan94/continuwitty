...
- **Go migration current checkpoint (2026-02-26):** CP118 completed in `checkpoint-go-migration.md` with `chat.unpin_engram` MCP dispatch in Go compatibility mode across direct and `tools/call` paths (with required UUID validation and runtime unpin adapter wiring), full `go test ./...` pass, and CodeScene safeguard pass.
