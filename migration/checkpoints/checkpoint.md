...
- **Phase 39 in progress:** temporal query + cost-aware context baseline shipped with engagement filters (`access_count_min`, `freshness_score_min`), recall/timeline windows (`last_accessed_after/before`, `freshness_computed_after/before`), and bounded `context_token_budget` controls across REST/MCP chat send paths; retrieval audit captures budget diagnostics (`context_token_budget`, `context_token_estimate`, `context_token_truncated`) while trace-aware relation/depth windows remain.
...
- [ ] Add trace-aware temporal filters and expanded contract/benchmark coverage.
