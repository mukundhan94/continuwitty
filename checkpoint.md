...
- **Go migration current checkpoint (2026-02-27):** CP136 completed in `checkpoint-go-migration.md` with MCP engram collection mutation dispatch baseline (`engram.collection_create`, `engram.collection_update`, `engram.collection_delete`, `engram.collection_add_items`, `engram.collection_remove_items`) in Go compatibility mode across direct and `tools/call` paths, runtime memory-admin collection mutation wiring with owner/admin visibility parity, project/duplicate/stale/not-found error mapping parity, full `go test ./...` pass, and CodeScene safeguard pass.
