...
- **Go migration current checkpoint (2026-02-27):** CP171 completed in `checkpoint-go-migration.md` with runtime cutover baseline (Go API `Dockerfile` + compose `api` service switched from Python build to Go build + non-Python healthcheck), full `go test ./...` pass, and CodeScene safeguard pass.
