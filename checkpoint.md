...
- **Go migration current checkpoint (2026-02-26):** CP112 completed in `checkpoint-go-migration.md` with `chat.get_lifecycle_policy` MCP dispatch in Go compatibility mode across direct and `tools/call` paths (with required UUID validation and session-based lifecycle policy projection), full `go test ./...` pass, and CodeScene safeguard pass.
