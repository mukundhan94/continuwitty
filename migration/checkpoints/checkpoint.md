...
- **Phase 40 in progress:** autonomous memory suggestions now include schema/model/repository foundations, admin REST + MCP list/action parity (`engram.curation_list`, `engram.curation_action`), and deterministic generation hooks from consolidation/contradiction refresh workflows into curation suggestions; dedicated acceptance + benchmark quality coverage remains.
...
- [ ] Add acceptance + benchmark coverage for suggestion quality and action latency.
