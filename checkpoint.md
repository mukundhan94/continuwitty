...
- **Go migration current checkpoint (2026-02-26):** CP125 completed in `checkpoint-go-migration.md` with `chat.delete_session` MCP dispatch in Go compatibility mode across direct and `tools/call` paths (with required `session_id`, optional delete settings, runtime memory-admin adapter wiring, and not-found parity mapping), full `go test ./...` pass, and CodeScene safeguard pass.
