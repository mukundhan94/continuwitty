...
- **Go migration current checkpoint (2026-02-26):** CP133 completed in `checkpoint-go-migration.md` with `engram.delete` and `engram.restore` MCP dispatch in Go compatibility mode across direct and `tools/call` paths, runtime memory-admin mutation wiring with owner/admin visibility parity, `-32004` not-found mapping with `engram_id`, full `go test ./...` pass, and CodeScene safeguard pass.
