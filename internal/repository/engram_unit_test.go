...
func queryWhereExpectedFragments() []string {
	return []string{
		...
		"COALESCE(useful_count, 0) <= $9",
		"COALESCE(access_count, 0) >= $10",
		"COALESCE(access_count, 0) <= $11",
		"COALESCE(feedback_count, 0) >= $12",
		"COALESCE(feedback_count, 0) <= $13",
		"COALESCE(contradiction_count, 0) >= $14",
		"COALESCE(contradiction_count, 0) <= $15",
		"THEN 0.0",
		"ELSE COALESCE(contradiction_count, 0)::DOUBLE PRECISION /",
		") >= $16",
		") <= $17",
		"COALESCE(freshness_score, 1.0) >= $18",
		"COALESCE(freshness_score, 1.0) <= $19",
		"CASE",
		"THEN 0.5",
		...
		") >= $20",
		") <= $21",
		"COALESCE(avg_relevance_feedback, 0.5) >= $22",
		"COALESCE(avg_relevance_feedback, 0.5) <= $23",
		"COALESCE(source_session_quality_score, 0.5) >= $24",
		"COALESCE(source_session_quality_score, 0.5) <= $25",
		"COALESCE(last_accessed_at, created_at) >= $26",
		"COALESCE(last_accessed_at, created_at) <= $27",
		"COALESCE(freshness_last_computed_at, created_at) >= $28",
		"COALESCE(freshness_last_computed_at, created_at) <= $29",
		"EXISTS (",
		"link.status = 'active'",
		"link.relation_type = $30",
		"owner_user_id = $3",
