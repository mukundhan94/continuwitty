...
- **Go migration current checkpoint (2026-02-27):** CP175 completed in `checkpoint-go-migration.md` with CI cutover baseline (GitHub Actions Go backend pipeline for `gofmt` + `go vet` + `go test`, plus migration checklist cutover markers updated), full `go test ./...` pass, and CodeScene safeguard pass.
