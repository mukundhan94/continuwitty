...
- **Go migration current checkpoint (2026-02-27):** CP140 completed in `checkpoint-go-migration.md` with MCP token project-scope dispatch parity for project-scoped tools (single-project autofill, multi-project explicit-selection guard, and allowlist enforcement), plus migrated chat-session token policy tests, full `go test ./...` pass, and CodeScene safeguard pass.
