...
@dataclass(frozen=True)
class ChatDispatchDependencies:
    chat_service: Any
...
def dispatch_chat_pinning_tool(
    *,
    dependencies: ChatDispatchDependencies,
    context: ChatDispatchContext,
) -> dict[str, Any] | None:
    normalized_method = (
        "chat.pin_engram" if context.method == "engram.pin_to_session" else context.method
    )

    def _list_pinned_engrams() -> dict[str, Any]:
        pinned = dependencies.chat_service.list_pinned_engrams(
            actor_user_id=context.actor_user_id,
            session_id=dependencies.parse_uuid(context.params, "session_id"),
        )
    ...
    def _pin_engram() -> dict[str, Any]:
        pinned = dependencies.chat_service.pin_engram(
            actor_user_id=context.actor_user_id,
            session_id=dependencies.parse_uuid(context.params, "session_id"),
    ...
    def _unpin_engram() -> dict[str, Any]:
        dependencies.chat_service.unpin_engram(
            actor_user_id=context.actor_user_id,
            session_id=dependencies.parse_uuid(context.params, "session_id"),
            engram_id=dependencies.parse_uuid(context.params, "engram_id"),
        )
    ...
    def _list_pinned_documents() -> dict[str, Any]:
        pinned = dependencies.chat_service.list_pinned_documents(
            actor_user_id=context.actor_user_id,
            session_id=dependencies.parse_uuid(context.params, "session_id"),
        )
    ...
    def _pin_document() -> dict[str, Any]:
        pinned = dependencies.chat_service.pin_document(
            actor_user_id=context.actor_user_id,
            session_id=dependencies.parse_uuid(context.params, "session_id"),
            payload=PinDocumentRequest(
                document_id=dependencies.parse_uuid(context.params, "document_id")
            ),
        )
        return {"pinned": pinned.model_dump(mode="json")}

    def _unpin_document() -> dict[str, Any]:
        dependencies.chat_service.unpin_document(
            actor_user_id=context.actor_user_id,
            session_id=dependencies.parse_uuid(context.params, "session_id"),
            document_id=dependencies.parse_uuid(context.params, "document_id"),
        )
...
def dispatch_chat_session_query_tool(
    *,
    ...
    def _list_sessions() -> dict[str, Any]:
        sessions = dependencies.chat_service.list_sessions(
            actor_user_id=context.actor_user_id,
    ...
    def _get_lifecycle_policy() -> dict[str, Any]:
        policy = dependencies.chat_service.get_lifecycle_policy(
            actor_user_id=context.actor_user_id,
            session_id=dependencies.parse_uuid(context.params, "session_id"),
        )
    ...
    def _update_lifecycle_policy() -> dict[str, Any]:
        policy = dependencies.chat_service.update_lifecycle_policy(
            actor_user_id=context.actor_user_id,
            session_id=dependencies.parse_uuid(context.params, "session_id"),
            payload=ChatLifecyclePolicyUpdateRequest(
                autosave_enabled=context.params.get("autosave_enabled"),
                autosave_strategy=context.params.get("autosave_strategy"),
                autosave_interval_minutes=context.params.get("autosave_interval_minutes"),
                autosave_min_messages=context.params.get("autosave_min_messages"),
                retention_days=context.params.get("retention_days"),
                retention_max_snapshots=context.params.get("retention_max_snapshots"),
            ),
        )
        return {"lifecycle_policy": policy.model_dump(mode="json")}

    def _list_messages() -> dict[str, Any]:
        messages = dependencies.chat_service.list_messages(
            actor_user_id=context.actor_user_id,
            session_id=dependencies.parse_uuid(context.params, "session_id"),
            limit=int(context.params.get("limit", 200)),
            offset=int(context.params.get("offset", 0)),
        )
        return {"messages": [item.model_dump(mode="json") for item in messages]}

    def _list_timeline() -> dict[str, Any]:
        events = dependencies.chat_service.list_timeline_events(
            actor_user_id=context.actor_user_id,
            session_id=dependencies.parse_uuid(context.params, "session_id"),
...
def dispatch_chat_primary_tool(
    *,
    ...
    def _list_project_documents() -> dict[str, Any]:
        ...
        documents = dependencies.ingestion_service.list_documents(
            actor_user_id=context.actor_user_id,
            project_id=context.params.get("project_id"),
            limit=int(context.params.get("limit", 200)),
            offset=int(context.params.get("offset", 0)),
    ...
    handlers = {
        "chat.create_session": lambda: {
            "session": dependencies.chat_service.create_session(
                actor_user_id=context.actor_user_id,
                payload=ChatSessionCreateRequest(**context.params),
            ).model_dump(mode="json")
        },
        "chat.send_message": lambda: {
            "message": dependencies.chat_service.send_message(
                actor_user_id=context.actor_user_id,
                session_id=dependencies.parse_uuid(context.params, "session_id"),
                payload=ChatMessageCreateRequest(
                    content_text=context.params.get("content_text", "")
                ),
            ).model_dump(mode="json")
        },
        "chat.list_project_documents": _list_project_documents,
        "chat.save_as_engram": lambda: dependencies.dispatch_chat_save_as_engram_tool(
            actor_user_id=context.actor_user_id,
        ...
        "chat.continue_session": lambda: {
            "continuation": dependencies.chat_service.continue_session(
                actor_user_id=context.actor_user_id,
                session_id=dependencies.parse_uuid(context.params, "session_id"),
                payload=ContinueSessionRequest(title=context.params.get("title")),
            ).model_dump(mode="json")
        },
    }
    handler = handlers.get(context.method)
