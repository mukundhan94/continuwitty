...
func registerChatPrimaryToolHandlers(handlers map[string]implementedToolHandler) {
	...
	handlers["chat.save_as_engram"] = chatSaveSessionAsEngramHandler()
...
func chatSaveSessionAsEngramHandler() implementedToolHandler {
	return func(
		service *CompatibilityService,
		ctx context.Context,
		actor Actor,
		...
		return service.dispatchChatSaveSessionAsEngramTool(ctx, actor, params)
	}
}
