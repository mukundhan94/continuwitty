...
- **Go migration current checkpoint (2026-02-26):** CP122 completed in `checkpoint-go-migration.md` with `chat.create_session` MCP dispatch in Go compatibility mode across direct and `tools/call` paths (with required `project_id`/`title`, enum validation, and runtime session-create adapter wiring), full `go test ./...` pass, and CodeScene safeguard pass.
