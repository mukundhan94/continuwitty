...
- **Go migration current checkpoint (2026-02-26):** CP128 completed in `checkpoint-go-migration.md` with `chat.save_as_engram` MCP dispatch in Go compatibility mode across direct and `tools/call` paths, runtime session-save wiring, defaults/validation parity for visibility + metadata fields, full `go test ./...` pass, and CodeScene safeguard pass.
