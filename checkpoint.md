...
- **Go migration current checkpoint (2026-02-27):** CP148 completed in `checkpoint-go-migration.md` with MCP `chat.save_as_engram` no-session missing-field payload parity (`data.missing=conversation_markdown`), migrated validation-data tests, full `go test ./...` pass, and CodeScene safeguard pass.
