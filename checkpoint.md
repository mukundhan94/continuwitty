...
- **Go migration current checkpoint (2026-02-27):** CP141 completed in `checkpoint-go-migration.md` with MCP token project-scope parity extended to session-scoped and engram-scoped tools via dispatch-time project resolution checks, plus migrated scope-enforcement tests, full `go test ./...` pass, and CodeScene safeguard pass.
