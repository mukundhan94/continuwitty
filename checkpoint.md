...
- **Go migration current checkpoint (2026-02-27):** CP170 completed in `checkpoint-go-migration.md` with MCP stream-route token project-scope denial parity extension for optional-project read tools (`chat.list_sessions`, `engram.query`, `engram.list`, `chat.list_project_documents`, `engram.collection_list`) across `tools/call` and direct methods, full `go test ./...` pass, and CodeScene safeguard pass.
