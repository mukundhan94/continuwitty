...
- **Go migration current checkpoint (2026-02-26):** CP109 completed in `checkpoint-go-migration.md` with `user.list_projects` MCP dispatch in Go compatibility mode across direct and `tools/call` paths using session-derived sorted project IDs, full `go test ./...` pass, and CodeScene safeguard pass.
