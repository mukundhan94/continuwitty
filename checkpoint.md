...
- **Go migration current checkpoint (2026-02-26):** CP101 completed in `checkpoint-go-migration.md` with MCP stream transport baseline in Go (`/api/v1/mcp/stream` GET/HEAD/POST), compatibility JSON-RPC service (`initialize`, `tools/list`, notification handling), runtime/router dependency wiring, full `go test ./...` pass, and CodeScene safeguard pass.
