...
- **Go migration current checkpoint (2026-02-27):** CP147 completed in `checkpoint-go-migration.md` with MCP conversation-create enrichment-origin parity for `chat.save_as_engram` fallback (`mcp.chat.save_as_engram`) and explicit `engram.create_from_conversation` origin wiring, migrated tests, full `go test ./...` pass, and CodeScene safeguard pass.
