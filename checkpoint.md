...
- **Go migration current checkpoint (2026-02-27):** CP176 completed in `checkpoint-go-migration.md` with acceptance-parity hardening for MCP conversation create (repository-level nil `tags`/`keywords` normalization plus deterministic fill-empty metadata enrichment for `abstract`/`tags`/`keywords`), full `go test ./...` pass, and CodeScene safeguard pass.
