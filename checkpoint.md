...
- **Go migration current checkpoint (2026-02-26):** CP104 completed in `checkpoint-go-migration.md` with MCP catalog visibility baseline in Go (`tools/list` population, read/write scope filtering, allowed-tools policy filtering, and clearer `tools/call` error semantics), full `go test ./...` pass, and CodeScene safeguard pass.
