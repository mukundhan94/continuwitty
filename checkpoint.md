...
- **Go migration current checkpoint (2026-02-26):** CP120 completed in `checkpoint-go-migration.md` with `chat.unpin_document` MCP dispatch in Go compatibility mode across direct and `tools/call` paths (with required UUID validation and runtime unpin-document adapter wiring), full `go test ./...` pass, and CodeScene safeguard pass.
