...
- **Go migration current checkpoint (2026-02-26):** CP130 completed in `checkpoint-go-migration.md` with `engram.query` and `engram.rehydrate` MCP dispatch in Go compatibility mode across direct and `tools/call` paths, runtime repository-backed query/rehydrate adapter wiring, query validation/defaults parity, and rehydrate not-found error parity (`-32004`), full `go test ./...` pass, and CodeScene safeguard pass.
