...
- **Go migration current checkpoint (2026-02-27):** CP163 completed in `checkpoint-go-migration.md` with MCP stream-route token optional-project autofill parity for `chat.list_project_documents` across `tools/call` and direct methods (single-project autofill + multi-project explicit-project guard with reason payload), full `go test ./...` pass, and CodeScene safeguard pass.
