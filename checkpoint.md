...
- **Go migration current checkpoint (2026-02-26):** CP107 completed in `checkpoint-go-migration.md` with project-default MCP dispatch in Go compatibility mode (`project.get_default` + `project.set_default`) across direct and `tools/call` paths, full `go test ./...` pass, and CodeScene safeguard pass.
