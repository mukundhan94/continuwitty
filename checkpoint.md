...
- **Go migration current checkpoint (2026-02-27):** CP180 completed in `checkpoint-go-migration.md` with shadow-routing parity workflow (`make shadow-compare` via compose `shadow` profile + nginx `/go`/`/py` proxy + status-family comparison script), updated migration checklist reverse-proxy/shadow markers, full `go test ./...` pass, and CodeScene safeguard pass.
