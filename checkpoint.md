...
- **Go migration current checkpoint (2026-02-26):** CP119 completed in `checkpoint-go-migration.md` with `chat.pin_document` MCP dispatch in Go compatibility mode across direct and `tools/call` paths (with required UUID validation and runtime pin-document adapter wiring), full `go test ./...` pass, and CodeScene safeguard pass.
