...
- **Go migration current checkpoint (2026-02-27):** CP155 completed in `checkpoint-go-migration.md` with MCP stream-route token project-fallback direct-method parity for `engram.create_from_conversation` and no-session `chat.save_as_engram` (single-project autofill + multi-project explicit-project validation), full `go test ./...` pass, and CodeScene safeguard pass.
