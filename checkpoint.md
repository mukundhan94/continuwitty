...
- **Go migration current checkpoint (2026-02-27):** CP154 completed in `checkpoint-go-migration.md` with MCP stream-route token project-fallback parity extension for no-session `chat.save_as_engram` (single-project autofill + multi-project explicit-project validation) using table-driven route tests, full `go test ./...` pass, and CodeScene safeguard pass.
