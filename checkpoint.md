...
- **Go migration current checkpoint (2026-02-26):** CP113 completed in `checkpoint-go-migration.md` with `chat.list_messages` MCP dispatch in Go compatibility mode across direct and `tools/call` paths (with required session lookup, default paging, and runtime message-list wiring), full `go test ./...` pass, and CodeScene safeguard pass.
