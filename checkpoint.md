...
- **Go migration current checkpoint (2026-02-27):** CP145 completed in `checkpoint-go-migration.md` with MCP `chat.save_as_engram` no-session conversation fallback parity (conversation create + enrichment payload), migrated chat-save parity tests, full `go test ./...` pass, and CodeScene safeguard pass.
