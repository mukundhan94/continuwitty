...
- **Go migration current checkpoint (2026-02-27):** CP152 completed in `checkpoint-go-migration.md` with MCP stream-route token project-fallback parity tests for `engram.create` (single-project autofill + multi-project explicit-project validation), full `go test ./...` pass, and CodeScene safeguard pass.
