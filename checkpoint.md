...
- **Go migration current checkpoint (2026-02-26):** CP106 completed in `checkpoint-go-migration.md` with first real MCP tool dispatch in Go compatibility mode (`user.get_profile` + `project.list`), `tools/call` success-envelope parity for implemented tools, full `go test ./...` pass, and CodeScene safeguard pass.
