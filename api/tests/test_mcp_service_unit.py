...
def test_dispatch_engram_move_project_routes_memory_admin_service() -> None:
    service, _, _, memory_admin_service = _build_service()
    actor_user_id = uuid4()
    engram_id = uuid4()
    _mock_owned_engram(
    ...
    call_kwargs = memory_admin_service.move_engram.call_args.kwargs
    assert call_kwargs["engram_id"] == engram_id
    assert call_kwargs["actor_user_id"] == actor_user_id
    assert call_kwargs["actor_role"] == "user"
    payload = call_kwargs["payload"]
