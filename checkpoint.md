...
- **Go migration current checkpoint (2026-02-26):** CP129 completed in `checkpoint-go-migration.md` with `engram.list`, `engram.get`, and `engram.collection_list` MCP dispatch in Go compatibility mode across direct and `tools/call` paths, runtime memory-admin adapter wiring with owner/admin visibility parity, full `go test ./...` pass, and CodeScene safeguard pass.
