...
- **Go migration current checkpoint (2026-02-26):** CP135 completed in `checkpoint-go-migration.md` with `engram.move_project` MCP dispatch in Go compatibility mode across direct and `tools/call` paths, runtime memory-admin move wiring with owner/admin visibility parity, stale/project-resolution error mapping parity, full `go test ./...` pass, and CodeScene safeguard pass.
