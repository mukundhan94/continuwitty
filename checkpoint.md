...
- **Go migration current checkpoint (2026-02-27):** CP157 completed in `checkpoint-go-migration.md` with MCP stream-route token project-scope session-precedence parity for `chat.save_as_engram` with `session_id` across `tools/call` and direct methods, full `go test ./...` pass, and CodeScene safeguard pass.
