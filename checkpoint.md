...
- **Go migration current checkpoint (2026-02-27):** CP138 completed in `checkpoint-go-migration.md` with MCP `chat.send_message` streaming parity in Go compatibility mode (`mcp.event` progress frames + terminal result/error), direct and `tools/call` stream-path routing parity with non-stream fallback behavior preserved, runtime stream adapter wiring, full `go test ./...` pass, and CodeScene safeguard pass.
