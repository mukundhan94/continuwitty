...
- **Phase 38 in progress:** contradiction warning baseline, contradiction-alert persistence repository baseline, and contradiction review tooling across admin REST + MCP are shipped (trace metadata, chat/send/stream/MCP warning parity, contradiction-alert storage + refresh/list/resolve repository workflows, and admin/API/MCP maintenance routes); warning-quality benchmarks remain.
