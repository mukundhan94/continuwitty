...
- **Go migration current checkpoint (2026-02-26):** CP110 completed in `checkpoint-go-migration.md` with `chat.list_sessions` MCP dispatch in Go compatibility mode across direct and `tools/call` paths (with optional project filter and paging defaults), full `go test ./...` pass, and CodeScene safeguard pass.
