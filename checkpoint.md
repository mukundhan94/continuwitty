...
- **Go migration current checkpoint (2026-02-27):** CP153 completed in `checkpoint-go-migration.md` with MCP stream-route token project-fallback parity extension for `engram.create_from_conversation` (single-project autofill + multi-project explicit-project validation) using table-driven tests, full `go test ./...` pass, and CodeScene safeguard pass.
