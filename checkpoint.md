...
- **Go migration current checkpoint (2026-02-26):** CP124 completed in `checkpoint-go-migration.md` with `chat.continue_session` MCP dispatch in Go compatibility mode across direct and `tools/call` paths (with required `session_id`, optional title passthrough, runtime continuation adapter wiring, and not-found parity mapping), full `go test ./...` pass, and CodeScene safeguard pass.
