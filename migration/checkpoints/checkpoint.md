...
- **Phase 32 in progress:** `cw>` query protocol kickoff shipped with parser/runtime metadata baseline (`cw_plan_applied`) across REST/MCP send + stream payloads.
...
- [ ] Add access-aware federated cross-project graph traversal/ranking in retrieval assembly.
...
| 32 | ContinuWitty Query Protocol | In Progress |
