...
- **Go migration current checkpoint (2026-02-27):** CP159 completed in `checkpoint-go-migration.md` with MCP stream-route token `allowed_tools` parity extension for disallowed write-tool invocations (`chat.send_message`) across `tools/call` and direct methods, full `go test ./...` pass, and CodeScene safeguard pass.
