...
- **Go migration current checkpoint (2026-02-27):** CP143 completed in `checkpoint-go-migration.md` with MCP token project-scope parity extended to collection-scoped mutation tools via `collection_id`-derived project resolution, runtime collection-lookup adapter wiring, migrated policy tests, full `go test ./...` pass, and CodeScene safeguard pass.
