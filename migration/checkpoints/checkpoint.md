...
- **Go migration current checkpoint (2026-02-27):** CP183 completed in `checkpoint-go-migration.md` with Go-only runtime/tooling cleanup: removed legacy Python artifacts (`api/`, parity scripts, shadow proxy config, Python contract/benchmark outputs), updated `Makefile` and `docker-compose.yml` to Go-only flows, and updated migration/checkpoint doc links, with full `go test ./...` pass and CodeScene safeguard pass.
