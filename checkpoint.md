...
- **Go migration current checkpoint (2026-02-27):** CP142 completed in `checkpoint-go-migration.md` with MCP token project-scope edge-policy hardening for `chat.save_as_engram` session precedence and `engram.move_project` source/target allowlist enforcement parity, plus migrated edge-case tests, full `go test ./...` pass, and CodeScene safeguard pass.
